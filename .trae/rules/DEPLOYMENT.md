...
- [ ] SSH 免密登录已配置（`ssh ubuntu@43.157.240.68 "whoami"` 和 `ssh ubuntu@43.133.44.232 "whoami"` 无需密码）
- [ ] 雅加达应用层：Git 仓库已初始化（`/opt/linkchest/api` 存在且是 git 仓库）
- [ ] 雅加达应用层：Docker、PM2、Nginx 已安装
- [ ] 雅加达应用层：Redis 容器已运行
- [ ] 雅加达应用层：autossh 隧道已配置（systemd 服务）
- [ ] 新加坡数据层：Docker 已安装
- [ ] 新加坡数据层：PostgreSQL 容器已运行
- [ ] 新加坡数据层：安全组已开放 5432 端口给雅加达 IP
...
LinkChest 同时运行 **海外（global）** 和 **国内（china）** 两套服务器，均采用**应用层 + 数据层分离架构**：
...
| 组件 | 技术 | 海外部署方式 | 国内部署方式 |
|------|------|-------------|-------------|
| API 服务 | Express + TypeScript (tsx) | PM2 进程管理（雅加达） | PM2 进程管理（服务器A） |
| Web 前端 | Next.js | PM2 进程管理（雅加达） | PM2 进程管理（服务器A） |
| 数据库 | PostgreSQL 16 | Docker 容器（新加坡） | Docker 容器（服务器B） |
| 缓存 | Redis 7 | Docker 容器（雅加达） | 系统服务（服务器A） |
| 反向代理 | Nginx | 系统服务（雅加达） | 系统服务（服务器A） |
| 进程管理 | PM2 | 全局 npm 包（雅加达） | 全局 npm 包（服务器A） |
| 数据库隧道 | autossh | systemd 服务（雅加达→新加坡） | 内网直连（服务器A→服务器B） |
...
| 配置项 | 海外 (`.env.global`) | 国内 (`.env.china`) | 说明 |
|--------|---------------------|---------------------|------|
| **DATABASE_URL** | `postgresql://...127.0.0.1:5433/...` | `postgresql://...114.132.81.246:5432/...` | 海外通过 SSH 隧道(5433)，国内直连 |
| **REDIS_URL** | `redis://127.0.0.1:6379` | `redis://127.0.0.1:6379` | 均为本机 |
| **SHARE_BASE_URL** | `https://linkchest.net` | `http://43.136.82.88` | 海外用域名，国内用IP |
| **WEB_BASE_URL** | `https://linkchest.net` | `http://43.136.82.88` | 海外用域名，国内用IP |
| **CORS_ORIGIN** | `https://linkchest.net` | `http://43.136.82.88` | 海外用域名，国内用IP |
| **MARKET** | `global` | `china` | 环境标识 |
| **GOOGLE_CLIENT_ID** | `76720591248-...apps.googleusercontent.com` | - | 仅海外 |
| **APPLE_CLIENT_ID** | `com.linkchest.app.signin` | `cn.linkchest.app.signin` | 海外/国内不同 |
| **APPLE_KEY_ID** | `FD9C8GPAHQ` | `J9NM3WV9PW` | 海外/国内不同 Key |
| **PAYPAL_CLIENT_ID** | 有 | - | 仅海外 |
| **WECHAT_APP_ID** | - | 有 | 仅国内 |
| **ALIPAY_APP_ID** | - | `2021006160619404` | 仅国内，正式版 APPID |
| **ALIPAY_PRIVATE_KEY** | - | 有 | 仅国内，商户 RSA2 私钥（PKCS#8） |
| **ALIPAY_PUBLIC_KEY** | - | 有 | 仅国内，支付宝公钥（验签回调） |
| **CLOUDFLARE_WORKER_URL** | `https://...workers.dev` | `http://localhost:3001/...` | 海外直连，国内代理 |
...
```env
# 数据库（通过 SSH 隧道连接新加坡数据层）
DATABASE_URL="postgresql://linkchest:linkchest123@127.0.0.1:5433/linkchest?schema=public"
...
# 海外登录配置 - Google
GOOGLE_CLIENT_ID=76720591248-m75hgr8256hut3m5j835qhgmirr9bnuh.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=your-google-client-secret

# 海外登录配置 - Apple Sign In（海外版）
APPLE_CLIENT_ID=com.linkchest.app.signin
APPLE_TEAM_ID=5A7Q5Y3XNZ
APPLE_KEY_ID=FD9C8GPAHQ
APPLE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----
MIGTAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBHkwdwIBAQQg5wtUIUreJztlQQnP
R4QTTHVgZfEZ/jJ6qiNF2AudByOgCgYIKoZIzj0DAQehRANCAARhYESBf0Esf+0k
S8ztuJFQSydSHbBi0qKRymBnLxQT8rKJ3zpoomDifVhdQp++SC0hMXXz3W6jNjpr
hQz23pQG
-----END PRIVATE KEY-----"
...
**Web 前端环境变量**: `apps/web/.env.production`

```env
NEXT_PUBLIC_API_URL=/api
NEXT_PUBLIC_MARKET=global
NEXT_PUBLIC_GOOGLE_CLIENT_ID=76720591248-m75hgr8256hut3m5j835qhgmirr9bnuh.apps.googleusercontent.com
NEXT_PUBLIC_APPLE_CLIENT_ID=com.linkchest.app.signin
```
...
# 支付宝正式版配置（仅国内版）
ALIPAY_APP_ID="2021006160619404"
ALIPAY_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----"  # 商户 RSA2 私钥（PKCS#8），.env 中 \n 会被 unescapePem 还原
ALIPAY_PUBLIC_KEY="-----BEGIN PUBLIC KEY-----\n...\n-----END PUBLIC KEY-----"  # 支付宝公钥（验签回调用），从支付宝开放平台获取
ALIPAY_NOTIFY_URL="https://linkchest.cn/api/payments/alipay/notify"  # 异步通知地址

# 国内登录配置 - 微信
WECHAT_APP_ID=your-wechat-app-id

# 国内登录配置 - Apple Sign In（国内版）
APPLE_CLIENT_ID=cn.linkchest.app.signin
APPLE_TEAM_ID=5A7Q5Y3XNZ
APPLE_KEY_ID=J9NM3WV9PW
APPLE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----
MIGTAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBHkwdwIBAQQgnprFgb0jKJLTdfqM
mYK+5nrGGWnYmiWydOJLcTYZ80ugCgYIKoZIzj0DAQehRANCAARk9PqyUB/7DJVC
Dv/tD0Tg/k1kbiV3VlS6/6KMpdKxbSfRyrq85YYqzf36MJpO3C3hjmvZ3NZFHZ9v
KPg8oY6h
-----END PRIVATE KEY-----"
...
**⚠️ 海外是应用层 + 数据层分离架构，部署时必须同时考虑两台服务器。**

**方案一：统一入口脚本（推荐）**

```bash
# 从本地执行（自动 SSH 到雅加达 → git pull → 构建 → 重启）
bash deploy/deploy.sh global
```

**脚本行为**：
1. 雅加达应用层执行 `git pull` 获取最新代码
2. 新加坡数据层执行 `git pull` 获取最新代码（如有需要）
3. 在雅加达执行数据库迁移（通过 SSH 隧道连接新加坡数据库）
4. 在雅加达构建并重启服务
...
```bash
# 步骤1：更新应用层代码（雅加达）
ssh ubuntu@43.157.240.68 "cd /opt/linkchest/api && git pull"

# 步骤2：在应用层执行数据库迁移（通过 SSH 隧道）
ssh ubuntu@43.157.240.68 "cd /opt/linkchest/api/apps/api && npx prisma migrate deploy"

# 步骤3：在应用层构建并重启
ssh ubuntu@43.157.240.68 "cd /opt/linkchest/api && npm run build && pm2 restart linkchest-api"
```

**海外部署专属注意事项**：
- **SSH 隧道必须运行**：autossh systemd 服务保持 5433 → 新加坡 5432 隧道
- **数据库操作在雅加达执行**：通过本地 5433 端口连接新加坡数据库
- **新加坡只运行 PostgreSQL**：不运行 API、WEB、Redis、Nginx
- **新加坡安全组最小化**：只开放 22（SSH）和 5432（给雅加达 IP）
...
海外应用层（雅加达）通过 SSH 隧道连接数据层（新加坡）的 PostgreSQL。

**隧道配置**：
```bash
# 隧道状态检查
ssh ubuntu@43.157.240.68 "sudo systemctl status autossh-tunnel"

# 手动重启隧道
ssh ubuntu@43.157.240.68 "sudo systemctl restart autossh-tunnel"

# 查看隧道日志
ssh ubuntu@43.157.240.68 "sudo journalctl -u autossh-tunnel -f"
```

**隧道参数**：
- 本地端口：`127.0.0.1:5433`
- 远程目标：`127.0.0.1:5432`（新加坡本机 PostgreSQL）
- 连接方式：`autossh -M 0 -N -L 5433:127.0.0.1:5432 ubuntu@43.133.44.232`
- 自动重启：`systemd` 服务配置 `Restart=always`

**故障排查**：
| 现象 | 原因 | 解决 |
|------|------|------|
| 数据库连接失败 | 隧道未运行 | `sudo systemctl restart autossh-tunnel` |
| 隧道频繁断开 | 网络不稳定 | 检查 autossh 日志，确认 `ServerAliveInterval` 配置 |
| 端口被占用 | 已有其他进程占用 5433 | `sudo lsof -i :5433` 查找并终止 |
