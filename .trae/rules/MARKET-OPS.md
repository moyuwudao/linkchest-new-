...
>
> ⚠️ **重要：本文档为人工可读参考，所有运营配置的真相源（Single Source of Truth）为：**
> **`project/apps/mobile/market-config.json`**
>
> **修改运营配置时，必须同步更新 `market-config.json`，否则构建将失败。**
...
> **注意：** 支付宝登录、Facebook Login 已移除，后续不计划启用。
> **Web端规则：** 在APK版本基础上，国内海外版均增加 Apple 登录。
...
type AuthSource = 'email' | 'google' | 'apple' | 'wechat'
```
...
| `/api/auth/apple` | POST | Apple Sign In | global, china (iOS) |
...
- ✅ 根据 `authProviders.google` 动态显示 Google 登录按钮（海外版）
- ✅ 根据 `authProviders.apple` 动态显示 Apple 登录按钮（iOS）
- ✅ 根据 `authProviders.wechat` 动态显示微信登录按钮（国内版）
- ✅ 第三方登录区域仅在至少一个 Provider 启用时显示
- ✅ 通用 OAuth 登录处理函数 `handleOAuthLogin(provider, credential)`

> **注意：** Web端已移除支付宝登录和Facebook Login按钮。
...
- ✅ 通过 `getMarketConfig()` 获取市场配置
- ✅ 根据 `authProviders.google` 动态显示 Google 登录按钮（海外版 Android）
- ✅ 根据 `authProviders.wechat` 动态显示微信登录按钮（国内版 Android/iOS）
- ✅ 根据 `authProviders.apple` 动态显示 Apple 登录按钮（iOS）
...
| 版本 | 应启用 | 应禁用 |
|------|--------|--------|
| **国内版 (china)** | 微信登录 | Google、Apple |
| **海外版 (global)** | Google 登录 | 微信、Apple |

##### iOS 登录配置

| 版本 | 应启用 | 应禁用 |
|------|--------|--------|
| **国内版 (china)** | 微信登录、Apple Sign In | Google |
| **海外版 (global)** | Google 登录、Apple Sign In | 微信 |

**注意：**
- 支付宝登录、Facebook Login 已移除，后续不计划启用
- Web 端国内版保留微信登录
- Web 端海外版保留 Google+Apple 登录
...
| **海外版 (global)** | PayPal、Google Pay | 微信支付、支付宝、Apple IAP |

##### iOS 支付配置

| 版本 | 应启用 | 应禁用 |
|------|--------|--------|
| **国内版 (china)** | Apple Pay | 微信支付、支付宝、PayPal、Google Pay |
| **海外版 (global)** | Apple Pay | 微信支付、支付宝、PayPal、Google Pay |

> **注意：** iOS 端统一仅支持 Apple Pay，不区分国内外版本。
