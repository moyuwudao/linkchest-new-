...
| [CASE-S018](#case-s018-tsc-not-found-npm-install-production-导致-devdependencies-缺失) | `tsc: not found` — npm install --production 导致 devDependencies 缺失 | high | frequent | resolved |
| [CASE-S019](#case-s019-psql-not-found-海外版-postgresql-客户端未安装) | `psql: not found` — 海外版 PostgreSQL 客户端未安装 | high | rare | resolved |
| [CASE-S020](#case-s020-部署脚本路径缺少-project-子目录) | 部署脚本路径缺少 `project/` 子目录 | critical | occasional | resolved |
| [CASE-S021](#case-s021-ecosystemconfigjs-硬编码数据库-url-导致连接错误数据库) | ecosystem.config.js 硬编码数据库 URL 导致连接错误数据库 | critical | rare | resolved |
| [CASE-S022](#case-s022-nginx-配置被-sed-全局替换破坏) | Nginx 配置被 `sed` 全局替换破坏 | high | rare | resolved |
| [CASE-S023](#case-s023-cos-对象存储未配置导致上传失败) | COS 对象存储未配置导致上传失败 | critical | rare | new |
| [CASE-S024](#case-s024-env-文件未同步导致-cos-配置未加载) | `.env` 文件未同步导致 COS 配置未加载 | critical | rare | new |
| [CASE-S025](#case-s025-system_covers-表为空导致封面不可见) | `system_covers` 表为空导致系统封面不可见 | high | rare | new |
| [CASE-S026](#case-s026-nginx-静态资源路径缺少-project-子目录导致海外版-web-404) | Nginx 静态资源路径缺少 `project/` 子目录导致海外版 Web 404 | high | occasional | new |
| [CASE-S027](#case-s027-支付宝商家订单参数异常) | 支付宝支付提示"商家订单参数异常" | critical | frequent | resolved |
| [CASE-S028](#case-s028-支付宝-pem-密钥解析失败) | 支付宝 PEM 密钥解析失败（`DECODER routines::unsupported`） | critical | occasional | resolved |
| [CASE-S029](#case-s029-支付宝-sdk-沙箱环境未切换) | 支付宝 SDK 沙箱环境未切换导致正式版走沙箱 | critical | occasional | resolved |
...
## CASE-S018: `tsc: not found` — npm install --production 导致 devDependencies 缺失
...
TypeScript 是 `devDependency`，当使用 `--production` 标志时，npm 会跳过所有 devDependencies 的安装，导致 `tsc` 命令不可用。

| 安装方式 | devDependencies | 适用场景 |
|----------|----------------|----------|
| `npm install` | ✅ 安装 | 开发/构建环境 |
| `npm install --production` | ❌ 跳过 | 纯运行环境 |

### 解决

**步骤 1：移除 `--production` 标志**

```bash
# 错误（update-server.sh / update-server-cn.sh）
...
1. **部署脚本不使用 `--production`**：构建环境需要 devDependencies
2. **构建前检查**：在构建前验证 `tsc` 是否存在
3. **CI/CD 缓存**：正确配置 node_modules 缓存策略

---

## CASE-S019: `psql: not found` — 海外版 PostgreSQL 客户端未安装
...
**海外版应用层服务器（雅加达）未安装 PostgreSQL 客户端。**

海外版架构为应用层+数据层分离：
- 应用层（雅加达）：API + WEB + Redis + Nginx
- 数据层（新加坡）：PostgreSQL

应用层需要 `psql` 客户端来测试通过 SSH 隧道的数据库连接，但 Ubuntu 默认不安装 PostgreSQL 客户端。

### 解决

**在应用层服务器安装 PostgreSQL 客户端：**

```bash
ssh ubuntu@43.157.240.68
sudo apt-get update
...
1. **服务器初始化时安装**：在 setup-server.sh 中添加 PostgreSQL 客户端安装
2. **部署脚本容错**：当 `psql` 不存在时，使用 `nc` 作为降级检查方式

---

## CASE-S020: 部署脚本路径缺少 `project/` 子目录
...
bash: deploy/update-server.sh: No such file or directory
```

同时，Nginx 静态资源路径也配置了错误的目录，导致 `_next/static/` 返回 404。
...
脚本错误地假设代码直接在 git 根目录下：
- `deploy.sh`: `UPDATE_SCRIPT="deploy/update-server.sh"` ❌
- `update-server.sh`: `BASE_DIR="/opt/linkchest/api"` ❌
- Nginx: `alias /opt/linkchest/api/apps/web/.next/static/` ❌
...
1. **部署前检查目录结构**：验证服务器上的实际路径
2. **使用统一的路径变量**：避免硬编码路径
3. **文档同步更新**：HIGH_RISK.md、DEPLOYMENT.md 中的路径与实际一致

---

## CASE-S021: ecosystem.config.js 硬编码数据库 URL 导致连接错误数据库
...
海外版部署后，API 健康检查返回 database error：

```json
{"status":"unhealthy","checks":{"database":"error","redis":"ok"}}
```

PM2 日志显示 API 正在连接 `114.132.81.246:5432`（国内数据层 IP）。

### 根因

**`ecosystem.config.js` 中硬编码了国内数据层的数据库 URL。**

```javascript
// deploy/ecosystem.config.js
cwd: '/opt/linkchest/api/apps/api',
env: {
  NODE_ENV: 'production',
  DATABASE_URL: 'postgresql://linkchest:LinkChest_DB_2026!@114.132.81.246:5432/linkchest',  // ❌ 国内数据层
  REDIS_URL: 'redis://localhost:6379',
},
```

这导致海外版 API 被强制连接到了国内数据库，而不是通过 SSH 隧道连接新加坡数据层。
...
**步骤 2：确保 start-api.sh 加载 .env 文件**
...
```bash
# 海外版 .env
DATABASE_URL=postgresql://linkchest:LinkChest_DB_2026!@127.0.0.1:5433/linkchest?schema=public

# 国内版 .env
DATABASE_URL=postgresql://linkchest:LinkChest_DB_2026!@114.132.81.246:5432/linkchest?schema=public
```

### 预防

1. **不在 ecosystem.config.js 中硬编码环境变量**：所有环境变量通过 .env 文件管理
2. **部署前验证数据库连接**：检查 API 实际连接的数据库地址
3. **多环境配置隔离**：海外版和国内版使用独立的 .env 文件

---

## CASE-S022: Nginx 配置被 `sed` 全局替换破坏
...
修改 Nginx 配置后，服务出现 404/403 错误：

```bash
# 试图只修改 /api/ 的代理端口
sudo sed -i 's|proxy_pass http://127.0.0.1:3003;|proxy_pass http://127.0.0.1:3001;|g' /etc/nginx/sites-enabled/linkchest-cn
```
...
**`sed` 全局替换 (`g` 标志) 会替换文件中所有匹配的行，而不仅仅是目标 location。**

Nginx 配置中有多个 location 块使用相同的 `proxy_pass http://127.0.0.1:3003;`，`sed` 的 `g` 标志会全部替换。

### 解决

**方案 A：使用范围限定的 sed（不推荐，容易出错）**

```bash
# 错误：会替换所有匹配
sudo sed -i 's|proxy_pass http://127.0.0.1:3003;|proxy_pass http://127.0.0.1:3001;|g' nginx.conf

# 正确：限定在特定 location 块内
sudo sed -i '/location \/api\//,/proxy_buffering off;/{s|3003|3001|}' nginx.conf
```

**方案 B：使用配置文件模板（推荐）**

```bash
# 将 Nginx 配置作为模板文件放入 git
# 通过部署脚本渲染正确的端口配置
cat > /etc/nginx/sites-enabled/linkchest-cn << 'EOF'
location /api/ {
...
**方案 C：使用 base64 编码传递完整配置（最可靠）**

```bash
# 将正确的配置文件编码为 base64，在服务器上解码
echo '<base64-encoded-config>' | base64 -d | sudo tee /etc/nginx/sites-enabled/linkchest-cn
sudo nginx -t && sudo nginx -s reload
```

### 预防

1. **避免使用 sed 全局替换 Nginx 配置**：容易误替换
2. **使用配置模板文件**：通过变量替换生成配置
3. **修改后验证**：`sudo nginx -t` 检查语法，`curl` 验证功能
4. **备份原配置**：修改前 `cp nginx.conf nginx.conf.bak`

## CASE-S023: COS 对象存储未配置导致上传失败
...
```json
{
  "code": "UPLOAD_COS_NOT_CONFIGURED",
  "message": "对象存储服务未配置，无法上传图片"
}
```
...
Error: COS_NOT_CONFIGURED
    at getCosClient (/opt/linkchest/api/project/apps/api/src/services/cos.ts:33:11)
```

### 根因

**服务器 `.env` 环境变量文件中缺少 COS（腾讯云对象存储）配置。**

项目使用腾讯云 COS SDK (`cos-nodejs-sdk-v5`) 进行对象存储操作：
- 封面图片上传 (`covers/*.webp`)
- 头像上传 (`avatars/*.webp`)
- 签名 URL 生成（用于图片访问）

代码中通过 `config.ts` 读取环境变量：
```typescript
const COS_CONFIG = {
...
当 `.env` 中缺少 `COS_SECRET_ID`、`COS_SECRET_KEY`、`COS_BUCKET` 任意一项时：
- `isCosConfigured()` 返回 `false`
- `getCosClient()` 抛出 `COS_NOT_CONFIGURED` 错误
- 所有上传/删除/签名操作均失败

**为什么部署后会缺失？**
- `.env` 文件在服务器上手动维护，不在 Git 中
- 部署时只关注 `git pull` 代码更新，忽略了环境变量完整性检查
- 没有标准化的 `.env` 初始化流程
...
在腾讯云控制台 → 轻量对象存储 → 存储桶详情页，获取：

| 配置项 | 说明 | 示例 |
|--------|------|------|
| SecretId | API 密钥 ID | `AKIDxxxxxxxxxxxxxxxx` |
| SecretKey | API 密钥 | `xxxxxxxxxxxxxxxxxxxx` |
| Bucket | 存储桶名称 | `linkchest-xinjiapo-1418307522` |
| Region | 存储桶地域 | `ap-singapore` / `ap-nanjing` |

**步骤 3：写入环境变量文件**

```bash
# 海外服务器
ssh ubuntu@43.157.240.68 "cat >> /opt/linkchest/api/project/apps/api/.env.global << 'EOF'
...
# 国内服务器
ssh ubuntu@43.136.82.88 "cat >> /opt/linkchest/api/project/apps/api/.env.china << 'EOF'
...
```bash
# 必须使用 --update-env 才能重新加载环境变量
ssh ubuntu@43.157.240.68 "pm2 restart linkchest-api --update-env"
ssh ubuntu@43.136.82.88 "pm2 restart linkchest-api --update-env"
```
...
```bash
# 在服务器上执行
ssh ubuntu@43.157.240.68 "cat > /tmp/test-cos.js << 'EOF'
const COS = require('cos-nodejs-sdk-v5');
const client = new COS({SecretId: process.env.COS_SECRET_ID, SecretKey: process.env.COS_SECRET_KEY});
client.getBucketLocation({Bucket: process.env.COS_BUCKET, Region: process.env.COS_REGION}, (err, data) => {
  if (err) { console.log('COS_ERROR:', err.message); process.exit(1); }
  else { console.log('COS_OK:', data.Location); process.exit(0); }
});
EOF
NODE_PATH=/opt/linkchest/api/project/node_modules node /tmp/test-cos.js"
```

### 预防

#### 方案 A：部署前环境变量完整性检查（推荐）

在 `deploy.sh` 或 `update-server.sh` 中增加检查：
...
check_env_vars() {
    local env_file=$1
    local required_vars=("DATABASE_URL" "JWT_SECRET" "REDIS_URL" "COS_SECRET_ID" "COS_SECRET_KEY" "COS_BUCKET" "COS_REGION")
    local missing_vars=()
    ...
    if [ ${#missing_vars[@]} -gt 0 ]; then
        echo "❌ 错误：环境变量文件缺少以下必填项："
        printf '  - %s\n' "${missing_vars[@]}"
        echo "📋 请先在 $env_file 中配置这些变量后再部署"
        exit 1
    fi
...
# 调用检查
check_env_vars "/opt/linkchest/api/project/apps/api/.env.global"
```

#### 方案 B：API 启动时环境变量校验

在 `apps/api/src/index.ts` 或 `config.ts` 中增加启动检查：
...
```bash
# init-env.sh
ENV_TYPE=${1:-global}  # global 或 china
...
# 1. 健康检查
curl -s http://127.0.0.1:3001/api/health | grep -q '"database":"ok"' && echo "✅ 数据库正常" || echo "❌ 数据库异常"

# 2. COS 配置检查
curl -s http://127.0.0.1:3001/api/health | grep -q '"cos":"configured"' && echo "✅ COS 已配置" || echo "⚠️ COS 未配置"

# 3. 上传测试（可选）
# curl -X POST -F "file=@test.webp" http://127.0.0.1:3001/api/upload/test
```

### 相关

- CASE-S003: 环境变量未加载（通用环境变量问题）
- CASE-S021: ecosystem.config.js 硬编码数据库 URL（环境变量管理问题）
- HIGH_RISK.md §3.5: 环境变量完整性检查

---

## CASE-S024: `.env` 文件未同步导致 COS 配置未加载
...
**`start-api.sh` 启动脚本只加载 `.env` 文件，不加载 `.env.global` 或 `.env.china`。**

服务器上环境变量分散在两个文件中：
- `.env.global`（海外）/ `.env.china`（国内）：包含 COS、数据库等所有配置
- `.env`：启动脚本实际加载的文件

当 `.env.global` / `.env.china` 中有 COS 配置，但 `.env` 中没有时：
1. `start-api.sh` 执行 `source .env`，只加载 `.env` 中的变量
2. API 进程启动后，`process.env.COS_SECRET_ID` 为 `undefined`
3. `isCosConfigured()` 返回 `false`
4. 上传接口返回 `UPLOAD_COS_NOT_CONFIGURED`

**为什么 `.env.global` 中有配置但 `.env` 中没有？**
- `.env.global` / `.env.china` 是手动维护的备份/分类文件
- `.env` 是启动脚本实际使用的文件
- 更新 `.env.global` 后忘记同步到 `.env`
...
```bash
# 检查 .env.global（海外）或 .env.china（国内）
ssh ubuntu@43.157.240.68 "grep COS_ /opt/linkchest/api/project/apps/api/.env.global"
# 应有输出

# 检查 .env（启动脚本实际加载的文件）
ssh ubuntu@43.157.240.68 "grep COS_ /opt/linkchest/api/project/apps/api/.env"
# 如果无输出，说明需要同步
```
...
```bash
# 获取进程 PID
ssh ubuntu@43.157.240.68 "pm2 describe linkchest-api | grep pid"

# 检查进程环境变量
ssh ubuntu@43.157.240.68 "cat /proc/<PID>/environ | tr '\\0' '\\n' | grep COS_"
```

### 预防

#### 方案 A：修改启动脚本加载正确的文件（推荐）

修改 `deploy/start-api.sh`：
...
在 `deploy.sh` 或 `update-server.sh` 中增加：
...
- CASE-S003: 环境变量未加载（通用环境变量问题）
- CASE-S023: COS 对象存储未配置（`.env` 中确实没有配置）
- HIGH_RISK.md §3.5: 环境变量完整性检查

---

## CASE-S025: `system_covers` 表为空导致系统封面不可见
...
前端"选择封面"页面显示 **"暂无封面"**，但 COS 存储桶中 `system-covers/` 目录下有文件。
...
**数据库 `system_covers` 表为空，没有记录。**

系统封面加载流程：
1. 前端调用 `/api/upload/system-covers` API
2. API 从数据库 `system_covers` 表查询记录
3. 返回 `cosUrl`（签名 URL）给前端
4. 前端用 `<img src={cosUrl}>` 显示图片

当数据库表为空时，API 直接返回 `"暂无封面"`，不会访问 COS。
...
**步骤 1：确认 COS 中有 system-covers 文件**

```bash
ssh ubuntu@43.157.240.68 "cat > /tmp/list-cos.js << 'EOF'
const COS = require('cos-nodejs-sdk-v5');
const client = new COS({ SecretId: '...', SecretKey: '...' });
client.getBucket({
  Bucket: 'linkchest-xinjiapo-1418307522',
  Region: 'ap-singapore',
  Prefix: 'system-covers/',
  MaxKeys: 1000
}, (err, data) => {
  if (err) { console.log('Error:', err.message); return; }
  console.log('Files:', data.Contents ? data.Contents.length : 0);
});
EOF
NODE_PATH=/opt/linkchest/api/project/node_modules node /tmp/list-cos.js"
```

**步骤 2：同步 COS 文件到数据库**

```bash
ssh ubuntu@43.157.240.68 "cat > /tmp/sync-covers.js << 'EOF'
const COS = require('cos-nodejs-sdk-v5');
...
async function sync() {
  ...
  for (const item of res.Contents || []) {
    if (item.Key === 'system-covers/') continue;
    ...
    await prisma.systemCover.create({
      data: {
        cosKey: item.Key,
        cosUrl: url.Url,
        urlExpiresAt: new Date(Date.now() + 3600 * 24 * 7 * 1000),
        size: parseInt(item.Size),
        name: item.Key.replace('system-covers/', '').replace(/\.webp$/, ''),
        sortOrder: 0,
        format: 'webp'
      }
    });
...
EOF
cd /opt/linkchest/api/project/apps/api && NODE_PATH=/opt/linkchest/api/project/node_modules node /tmp/sync-covers.js"
```

**步骤 3：验证数据库记录**

```bash
ssh ubuntu@43.157.240.68 "PGPASSWORD=... psql -h 127.0.0.1 -p 5433 -U linkchest -d linkchest -c 'SELECT COUNT(*) FROM system_covers;'"
```
...
在 Prisma migration 或 seed 脚本中插入系统封面数据：

```typescript
// prisma/seed.ts
async function seedSystemCovers() {
  const covers = await listCosFiles('system-covers/');
  for (const cover of covers) {
    await prisma.systemCover.create({ data: cover });
  }
}
```
...
```bash
# 同步系统封面到数据库
node scripts/sync-system-covers.js
```

#### 方案 C：API 兜底逻辑

如果数据库为空，API 自动从 COS 列举并插入：

```typescript
async function getSystemCovers() {
  let covers = await prisma.systemCover.findMany();
  if (covers.length === 0) {
    // 自动从 COS 同步
    await syncSystemCoversFromCos();
    covers = await prisma.systemCover.findMany();
  }
  return covers;
}
```
...
- CASE-S023: COS 对象存储未配置（存储桶无文件）
- CASE-S024: `.env` 文件未同步（配置未加载）
- HIGH_RISK.md §3.5: 环境变量完整性检查

---

## CASE-S026: Nginx 静态资源路径缺少 `project/` 子目录导致海外版 Web 404
...
海外版 WEB 端部署后无法访问，页面白屏或加载异常，浏览器控制台显示静态资源 404：

```
GET https://linkchest.net/_next/static/chunks/4481-bae78f0f9598ea98.js net::ERR_ABORTED 404 (Not Found)
```

Nginx 错误日志显示文件不存在：
```
2026/06/02 10:43:56 [error] 952601#952601: *9274 open() "/opt/linkchest/api/apps/web/.next/static/chunks/4481-bae78f0f9598ea98.js" failed (2: No such file or directory)
```

### 根因

**Nginx 配置中 `/_next/static/` 的 alias 路径缺少 `project/` 子目录。**
...
```nginx
location /_next/static/ {
    alias /opt/linkchest/api/apps/web/.next/static/;  # ❌ 缺少 project/
}
```
...
```nginx
location ^~ /_next/static/ {
    alias /opt/linkchest/api/project/apps/web/.next/static/;  # ✅ 包含 project/
    expires 1y;
    access_log off;
...
**为什么国内版正常，海外版出问题？**
- 国内版 Nginx 配置已更新为正确路径
- 海外版 Nginx 配置未同步更新，仍使用旧路径
- 海外版服务器代码目录结构与国内一致（均在 `project/` 子目录下）
...
```bash
ssh ubuntu@43.157.240.68 "ls -la /opt/linkchest/api/project/apps/web/.next/static/chunks/ | head -5"
# 应能列出文件，确认路径正确
```
...
```bash
ssh ubuntu@43.157.240.68 "sudo sed -i 's|/opt/linkchest/api/apps/web/.next/static/|/opt/linkchest/api/project/apps/web/.next/static/|g' /etc/nginx/sites-enabled/linkchest"
```

**步骤 3：使用 `^~` 提升匹配优先级（如存在正则冲突）**

如果 Nginx 配置中存在 `location ~* \.(css|js)$` 等正则匹配，需要确保 `/_next/static/` 规则优先：

```nginx
# 必须使用 ^~ 前缀，确保此规则优先于正则匹配
location ^~ /_next/static/ {
    alias /opt/linkchest/api/project/apps/web/.next/static/;
    expires 1y;
    access_log off;
...
```bash
ssh ubuntu@43.157.240.68 "sudo nginx -t && sudo nginx -s reload"
```
...
```bash
# 测试静态资源访问
curl -s -o /dev/null -w "%{http_code}" https://linkchest.net/_next/static/chunks/main-app.js
# 预期：200

# 测试页面访问
curl -s -o /dev/null -w "%{http_code}" https://linkchest.net/
# 预期：200
```
...
在 Nginx 配置文件中使用变量：
```nginx
set $web_root /opt/linkchest/api/project/apps/web;
...
在 `deploy/update-server.sh` 中增加 Nginx 路径检查：
```bash
# 检查 Nginx 静态资源路径
if grep -q "/opt/linkchest/api/apps/web/.next/static/" /etc/nginx/sites-enabled/linkchest; then
    echo "⚠️ 警告：Nginx 静态资源路径缺少 project/，正在修正..."
    sudo sed -i 's|/opt/linkchest/api/apps/web/.next/static/|/opt/linkchest/api/project/apps/web/.next/static/|g' /etc/nginx/sites-enabled/linkchest
    sudo nginx -t && sudo nginx -s reload
fi
```
...
```bash
#!/bin/bash
# validate-nginx-static.sh

echo "=== Nginx 静态资源路径验证 ==="

# 检查路径是否包含 project/
if grep -q "project/apps/web/.next/static" /etc/nginx/sites-enabled/linkchest; then
    echo "✅ 静态资源路径包含 project/"
else
    echo "❌ 错误：静态资源路径可能缺少 project/"
    exit 1
fi
...
if grep -q "location ^~ /_next/static/" /etc/nginx/sites-enabled/linkchest; then
    ...
    echo "⚠️ 警告：/_next/static/ 未使用 ^~ 前缀，可能被正则匹配覆盖"
fi
...
- CASE-S013: Nginx location 匹配顺序问题（正则匹配优先级）
- CASE-S020: 部署脚本路径缺少 `project/` 子目录（同一根因的不同表现）
- HIGH_RISK.md §3.3: 海外部署检查清单（Nginx 配置验证）

---

## CASE-S027: 支付宝商家订单参数异常
...
支付宝 App 支付调用成功，但支付宝页面提示 **"商家订单参数异常，请重新发起付款"**。
...
| 编号 | 原因 | 说明 |
|------|------|------|
| 1 | **APPID 与 PID 混淆** | APPID 是应用 ID（如 `2021006160619404`），PID 是商家账号（如 `2088721102032150`），`app_id` 参数必须用 APPID |
| 2 | **缺少 `format: 'json'` 参数** | 支付宝 App 支付要求所有请求必须带 `format=json`，缺失则按 form 解析失败 |
| 3 | **timestamp 格式错误** | 支付宝要求 `yyyy-MM-dd HH:mm:ss`，不能用 `toISOString()`（含 `T` 和时区后缀） |
| 4 | **商户公钥和私钥不匹配** | 商户私钥签名请求，商户公钥上传到支付宝平台验签；如果密钥对不匹配，支付宝验签失败 |
| 5 | **orderString 参数未排序** | 签名时参数按字母排序，但 orderString 拼接时未排序，导致签名与请求不一致 |
...
```bash
# 检查 .env 中的 ALIPAY_APP_ID
ssh ubuntu@43.136.82.88 "grep ALIPAY_APP_ID /opt/linkchest/api/project/apps/api/.env"
# 正式版应为 2021006160619404，不是 PID 2088721102032150
```

**步骤 2：确认 buildCommonParams 包含 format**

```typescript
// alipay.ts - buildCommonParams 必须包含
{
  ...
  charset: 'utf-8',
  sign_type: 'RSA2',
  timestamp: 'yyyy-MM-dd HH:mm:ss',  // ✅ 正确格式
  version: '1.0',
  notify_url: '...',
}
```

**步骤 3：确认 timestamp 格式**
...
// 错误：toISOString
new Date().toISOString().replace('T', ' ').substring(0, 19)  // ❌ 时区不对
```

**步骤 4：确认商户公钥已上传到支付宝平台**

1. 登录 [支付宝开放平台](https://open.alipay.com)
2. 进入应用 → 开发设置 → 接口加签方式
3. 上传商户公钥（从商户私钥推导出的公钥）
4. 获取支付宝公钥（用于验签回调）

**步骤 5：确认 orderString 参数排序**

```typescript
// orderString 参数必须按字母排序（与签名一致）
...
1. **部署前验证支付宝配置**：检查 `.env` 中 ALIPAY_APP_ID、ALIPAY_PRIVATE_KEY、ALIPAY_PUBLIC_KEY
2. **密钥对验证**：确保商户私钥和上传到平台的公钥是同一对
3. **沙箱测试**：先用沙箱环境验证支付流程，再切换正式版
4. **支付宝仅国内版**：不要将支付宝配置部署到海外服务器

### 相关

- CASE-S028: 支付宝 PEM 密钥解析失败
- CASE-S029: 支付宝 SDK 沙箱环境未切换
- HIGH_RISK.md §2.2: 国内专属禁止行为（支付宝相关）
...
dotenv 不会自动将 `\\n` 转义为换行符，导致 PEM 字符串变成一整行，OpenSSL 无法解析。
...
# 代码读取后（\\n 未被还原，变成一整行）
"-----BEGIN PRIVATE KEY-----MIIEvw...-----END PRIVATE KEY-----"
...
1. **所有 PEM 密钥读取时必须经过 `unescapePem` 处理**
2. **.env 中 PEM 密钥用双引号包裹**，确保 `\n` 被保留
3. **部署后验证密钥格式**：`node -e "console.log(process.env.ALIPAY_PRIVATE_KEY?.includes('\\n'))"`

### 相关

- CASE-S027: 支付宝商家订单参数异常（密钥问题也会导致此错误）
...
- **沙箱测试时**：支付宝 SDK 默认走生产环境，导致沙箱数据和生产环境冲突，支付报错
- **正式上线时**：忘记移除沙箱环境设置，导致正式版走沙箱环境

### 根因

**支付宝 Android SDK 默认使用生产环境。沙箱测试时必须显式调用 `EnvUtils.setEnv(EnvUtils.EnvEnum.SANDBOX)`，正式版必须移除此调用。**

```java
// 沙箱测试时需要添加（AlipayPayModule.kt）
EnvUtils.setEnv(EnvUtils.EnvEnum.SANDBOX);  // 沙箱必须

// 正式版必须移除
// EnvUtils.setEnv(EnvUtils.EnvEnum.SANDBOX);  // ❌ 正式版必须删除
```
...
| 步骤 | 操作 | 文件 |
|------|------|------|
| 1 | 移除 `EnvUtils.setEnv(EnvUtils.EnvEnum.SANDBOX)` | `AlipayPayModule.kt` |
| 2 | 移除 `import com.alipay.sdk.app.EnvUtils` | `AlipayPayModule.kt` |
| 3 | 移除 `ALIPAY_SANDBOX` 环境变量 | 服务器 `.env` |
| 4 | 更新 `ALIPAY_APP_ID` 为正式版 | 服务器 `.env` |
| 5 | 更新 `ALIPAY_PUBLIC_KEY` 为正式版支付宝公钥 | 服务器 `.env` |
| 6 | 确认网关为 `https://openapi.alipay.com/gateway.do` | `alipay.ts` |
| 7 | 重新构建 APK | WSL |

### 预防

1. **沙箱代码与正式版代码隔离**：沙箱相关代码用注释标记 `// SANDBOX ONLY`
2. **上线前检查清单**：确认 `AlipayPayModule.kt` 中无 `EnvUtils` 引用
3. **环境变量清理**：正式版 `.env` 中不应有 `ALIPAY_SANDBOX_*` 变量

### 相关

- CASE-S027: 支付宝商家订单参数异常
- HIGH_RISK.md §2.2: 国内专属禁止行为（支付宝 SDK 环境切换）

---

*最后更新：2026-06-06*
*版本：v1.12 — 新增 CASE-S027/S028/S029（支付宝支付异常案例）*
