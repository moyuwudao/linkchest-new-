...
router.post('/alipay', async (req, res) => {
  ...
  try {
    ...
    const { getAuthProvider } = await import('../providers/auth')
    const provider = await getAuthProvider('alipay_auth')
    
    if (!provider.isConfigured()) {
      logger.error({}, '[Alipay Auth] 支付宝登录未配置')
      return errorResponse(res, 500, AuthErrorCodes.SERVER_ERROR)
    }
    ...
    const alipayId = result.providerUserId
    const email = result.email || null
    const name = result.name || '用户'
    const picture = result.avatar

    let user = await prisma.user.findUnique({ where: { alipayId } })
    ...
    if (!user && email) {
      ...
      if (user) {
        ...
        user = await prisma.user.update({
          where: { id: user.id },
          data: { alipayId, lastLoginAt: new Date() },
        })
      }
    }
    ...
    if (!user) {
      ...
      if (referralCode && typeof referralCode === 'string') {
        try {
          ...
          const result = await useReferralCode(referralCode, user.id, clientIp, userAgent)
          if (result.success) {
            logger.info({ reqId, userId: user.id, code: referralCode.toUpperCase() }, '支付宝邀请码注册成功')
          }
        } catch (refErr) {
          logger.warn({ reqId, referralCode, err: (refErr as Error).message }, '支付宝邀请码处理失败')
        }
      }
    } else {
      if (user.status === 'banned') {
    ...
    const err = error as { message?: string }
    logger.error({ err: err.message }, '支付宝登录错误')
    res.locals.errorMessage = err.message
    return errorResponse(res, 401, AuthErrorCodes.INVALID_GOOGLE_TOKEN)
  }
})
...
router.post('/facebook', async (req, res) => {
  const reqId = req.reqId
  try {
    const { credential, lang = 'en', referralCode } = req.body
    if (!credential) {
      return errorResponse(res, 400, AuthErrorCodes.INVALID_GOOGLE_TOKEN)
    }

    const { getAuthProvider } = await import('../providers/auth')
    const provider = await getAuthProvider('facebook')

    if (!provider.isConfigured()) {
      logger.error({}, '[Facebook Auth] Facebook登录未配置')
      return errorResponse(res, 500, AuthErrorCodes.SERVER_ERROR)
    }
    ...
    const facebookId = result.providerUserId
    const email = result.email || null
    const name = result.name || '用户'
    const picture = result.avatar
    ...
    if (!user) {
      ...
      if (referralCode && typeof referralCode === 'string') {
        try {
          ...
          const result = await useReferralCode(referralCode, user.id, clientIp, userAgent)
          if (result.success) {
            logger.info({ reqId, userId: user.id, code: referralCode.toUpperCase() }, 'Facebook邀请码注册成功')
          }
        } catch (refErr) {
          logger.warn({ reqId, referralCode, err: (refErr as Error).message }, 'Facebook邀请码处理失败')
        }
      }
    } else {
      if (user.status === 'banned') {
    ...
    const err = error as { message?: string }
    logger.error({ err: err.message }, 'Facebook登录错误')
    res.locals.errorMessage = err.message
    return errorResponse(res, 401, AuthErrorCodes.INVALID_GOOGLE_TOKEN)
  }
})
