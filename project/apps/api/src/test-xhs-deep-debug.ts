...
;(async () => {
  ...
  try {
    ...
    const cookieObjs = xhsCookieEntry.cookies.map(c => ({
      name: c.name, value: c.value, domain: c.domain, path: c.path || '/', sameSite: 'Lax' as const,
    }))
    ...
    const status = await page.evaluate(() => {
      return {
        ...
        bodyTextSample: (document.body?.textContent || '').substring(0, 200).replace(/\s+/g, ' '),
        ogImage: document.querySelector('meta[property="og:image"]')?.getAttribute('content'),
        allMetaImages: Array.from(document.querySelectorAll('meta[property*="image" i]')).map(m => m.getAttribute('content') || '').filter(Boolean).slice(0, 5),
      }
    })
    ...
    const scripts = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('script'))
        ...
        .filter(t => t.includes('INITIAL_STATE') || t.includes('__INITIAL_STATE__'))
        .slice(0, 1)
    })
    console.log('找到 INITIAL_STATE 脚本数:', scripts.length)
    if (scripts.length > 0) {
      console.log('前 800 字符:', scripts[0].substring(0, 800))
    }
    ...
    console.log('ERROR:', e.message, e.stack?.substring(0, 500))
  } finally {
